...
the Nebul                    traces.append(trace)
                    packet_id += 1
        
        logger.info(f"Generated {len(traces)} uniform random packets")
        return traces

    def generate_vcd_testbench(self, output_path: str, traces: List[PacketTrace]) -> str:
...
module tb_nebula_top_traffic;
    ...
    task inject_traffic();
        ...
        // Read traffic pattern from generated stimulus file
        $readmemh("traffic_stimulus.txt", traffic_data);
    ...
    task inject_packet(int src_node, int dst_node, string pkt_type, int size);
        ...
        int src_x = src_node % {self.config.mesh_width};
        int src_y = src_node / {self.config.mesh_width};
        int dst_x = dst_node % {self.config.mesh_width};
        int dst_y = dst_node / {self.config.mesh_width};
...
'''
    ...
    def generate_stimulus_file(self, output_path: str, traces: List[PacketTrace]):
        ...
        with open(output_path, 'w') as f:
            ...
            for trace in traces:
                f.write(f"{{trace.timestamp:08d}} {{trace.source_node:02d}} {{trace.dest_node:02d}} ")
    ...
    def generate_pattern_traces(self, pattern: TrafficPattern) -> List[PacketTrace]:
        ...
        elif pattern == TrafficPattern.MATRIX_MULTIPLY:
            return self.generate_matrix_multiply_traffic()
        elif pattern == TrafficPattern.TRANSFORMER:
            return self.generate_transformer_traffic()
        else:
            logger.warning(f"Pattern {{pattern}} not implemented, using uniform random")
            return self.generate_uniform_random_traffic() interconnect system. It supports:
...
class NebulaTrafficGenerator:
    ...
    def __init__(self, config: TrafficConfig):
        ...
        logger.info(f"Mesh: {config.mesh_width}x{config.mesh_height} ({self.num_nodes} nodes)")
        ...
        for cycle in range(self.config.duration_cycles):
            for src_node in range(self.num_nodes):
                if self.random.random() < self.config.injection_rate:
                    ...
                    dst_node = self.random.choice([n for n in range(self.num_nodes) if n != src_node])
                    
                    # Choose packet type based on configuration
                    if self.config.enable_axi and self.config.enable_chi:
                        packet_type = self.random.choice(['AXI_READ', 'AXI_WRITE', 'CHI_READ', 'CHI_WRITE'])
        ...
        for cycle in range(self.config.duration_cycles):
            for src_node in range(self.num_nodes):
                if self.random.random() < self.config.injection_rate:
                    ...
                    if dst_x < self.config.mesh_width and dst_y < self.config.mesh_height:
    ...
    def generate_gpu_workload_traffic(self) -> List[PacketTrace]:
        ...
        for cycle in range(self.config.duration_cycles):
            for src_node in range(self.num_nodes):
                ...
                compute_nodes = [n for n in range(0, self.num_nodes, 2)]  # Even numbered nodes
                ...
                if self.random.random() < injection_prob:
                    ...
                    if self.random.random() < 0.7:
                        ...
                        for dx in range(-2, 3):
                            for dy in range(-2, 3):
                                ...
                                if (0 <= new_x < self.config.mesh_width and 
                                    0 <= new_y < self.config.mesh_height):
                                    node = self.coords_to_node(new_x, new_y)
                                    if node != src_node:
                                        nearby_nodes.append(node)
                        
                        dst_node = self.random.choice(nearby_nodes) if nearby_nodes else src_node
                    else:
                        # Distant destination
                        dst_node = self.random.choice([n for n in range(self.num_nodes) if n != src_node])
                    ...
                    if self.random.random() < 0.6:  # 60% AXI memory traffic
                        packet_type = self.random.choice(['AXI_READ', 'AXI_WRITE'])
                        size_bytes = self.config.packet_size_bytes * self.random.choice([1, 2, 4, 8])
    ...
    def generate_traffic(self) -> List[PacketTrace]:
        ...
        if self.config.pattern == TrafficPattern.UNIFORM_RANDOM:
            ...
            logger.warning(f"Pattern {self.config.pattern} not implemented, using uniform random")
            return self.generate_uniform_random_traffic()
    ...
    def analyze_performance(self, traces: List[PacketTrace]) -> PerformanceMetrics:
        ...
        if dest_counts:
            ...
            hotspot_congestion = max_dest_traffic / avg_dest_traffic if avg_dest_traffic > 0 else 1.0
        else:
        ...
        if dest_counts:
            ...
            load_balance_index = traffic_std / avg_dest_traffic if avg_dest_traffic > 0 else 0.0
        else:
...
''')
            ...
            for timestamp in sorted(traces_by_time.keys()):
                if timestamp > current_cycle:
                    ...
                    f.write(f'    repeat({timestamp - current_cycle}) @(posedge clk);\\n')
                    current_cycle = timestamp
                
                f.write(f'    // Inject packets at cycle {timestamp}\\n')
                for trace in traces_by_time[timestamp]:
                    f.write(f'    // Packet {trace.packet_id}: {trace.source_node} -> {trace.dest_node} ({trace.packet_type})\\n')
                    f.write(f'    inject_packet({trace.source_node}, {trace.dest_node}, {trace.size_bytes}, "{trace.packet_type}");\\n')
...
def create_visualizations(generator: NebulaTrafficGenerator, 
    ...
    ax1.axvline(metrics.avg_latency_cycles, color='red', linestyle='--', label=f'Avg: {metrics.avg_latency_cycles:.1f}')
    ax1.legend()
    ...
    metric_names = ['Throughput\\n(Gbps)', 'Network\\nUtilization', 'Hotspot\\nCongestion', 'Load Balance\\nIndex']
    metric_values = [metrics.throughput_gbps, metrics.network_utilization, 
                    metrics.hotspot_congestion, metrics.load_balance_index]
    ...
    for i, bar in enumerate(bars):
        ...
        ax3.text(bar.get_x() + bar.get_width()/2., height + 0.01,
                f'{metric_values[i]:.3f}', ha='center', va='bottom')
...
def main():
    ...
    parser.add_argument('--pattern', type=str, default='uniform_random',
    ...
    print(f"  Mesh: {config.mesh_width}x{config.mesh_height} ({generator.num_nodes} nodes)")
